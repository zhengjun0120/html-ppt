...
func (as *AgentService) TestChat(ctx context.Context, messages []openai.ChatCompletionMessageParamUnion) (string, error) {
...
const maxTurns = 20 //最大允许调用20次工具

func (as *AgentService) StreamChat(ctx context.Context, messages []openai.ChatCompletionMessageParamUnion, emit func(StreamEvent) error) error {
	opt := as.setChatOpts()
	...
	for i:=0;;i++{
		...
		for stream.Next(){
			...
			if !acc.AddChunk(chunk){
				if emitErr := emit(StreamEvent{Type: EventTypeError,Content: "数据累加失败"});emitErr!=nil{
					stream.Close()
			...
			if delta.Content!=""{
				fullText.Write([]byte(delta.Content))
