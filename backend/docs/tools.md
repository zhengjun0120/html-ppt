...
- **参数**：`{deck_id, pages?, focus?}`——pages 留空 = 只量测不产图（几秒、免费、随时可调）；
  点名页号才真正截图发给视觉模型（**一次 run 最多 3 次**，慢且贵）
- **focus：把主模型的疑问带进看图调用**。看图是一次隔离调用，拿不到主对话的上下文
  （模型换主模型也不受影响）；主模型在担心什么只有 focus 这条通道能传——
  "第 9 页数据卡与来源小字是否对齐"这样的具体疑问，报告会优先回答。
  这是"转述损耗"的补法，而不是把图片塞进主对话（图片一旦进对话，本轮 run 后续
  每次请求都要原样重运，24 轮的 generating 阶段账单会被放大一个数量级）
- **大纲意图随图注入**：点名看图时自动把每页的大纲摘要（标题+要点，`outlineBriefs`）
  附在对应截图前，看图才能审「内容切题」（这页放的不是它该讲的东西），不只是排版
- **结构化报告**：每条问题 = `页号｜硬/软｜类别｜具体是什么（引用页面文字）｜定位（元素+文字锚点）｜期望效果`
  ——「定位」让主模型回到 HTML 能直接找到那块，修复不用猜
...
- **看图**：图片与数字（全份量测 + 程序硬判定）一起给模型，产出可执行的修复清单；
  点名制：v1 实测每次全量看图 9 页要 2 分钟，点名的意义是把"看图"从默认动作
  变成可疑页的精确打击
