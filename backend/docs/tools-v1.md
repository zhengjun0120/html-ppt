# deck-v1 工具清单（历史档案，已下线）

> **⚠️ 本文档描述的是 deck-v1 管线，已随 2026-09 的 deck-v2 重构整体下线。**
> v1 工具（write_deck / update_theme / custom_css / read_component / read_icons 等）
> 已从代码中移除（`func_tool.go` 只剩 ask_user/历史/schema 等共用件），
> 旧格式 deck 在对话里会被明确拒绝（"旧格式文稿已随新版下线"）。
>
> **当前权威文档：[tools.md](tools.md)（deck-v2）。**
>
> 本文保留的原因：里面的工程纪律（schema 描述禁半角逗号、嵌入提示词 LF 归一化、
> 提示词缓存的前缀纪律、消毒/样式体检的分级处置、"错误即反馈"）在 v2 里仍然有效，
> 且大量判定函数（`sanitize.go` / `stylelint.go` / `variable_contract.go`）被 v2 写路径
> 原样复用；各节的设计推演也是 v2 决策的出处。阅读时请把"工具"理解为历史，
> 把"纪律与判定"理解为现状。
