...
适用 role：cover。
内容约束：主标题 ≤12 字；lede ≤24 字；kicker 是品牌短标签
...
内容约束：宣言 ≤28 字；pill 3-5 个、每个 ≤6 字
...
适用 role：data。
内容约束：3 个指标；指标名 ≤8 字；数字 ≤6 字符；说明 ≤24 字（无出处标“估算”）
...
适用 role：data。
内容约束：4-6 条；条目名 ≤10 字；宽度按真实比例给 style，数字标注在条目名后
...
适用 role：cta。
内容约束：Ask 一句话 ≤30 字；支持点 ≤3 条、每条 ≤14 字
...
用途：收尾致谢 + 联系方式 pill
适用 role：thanks / cta。
内容约束：大字 ≤8 字；lede ≤20 字；pill ≤2 个
