...
通用约定：
- 类名分两族：**base 原语**（`slide kicker h1 h2 h3 h4 lede dim dim2 mono stack row grid g2 g3 g4 center card card-accent card-soft card-outline pill pill-accent divider divider-accent mt-s mt-m mb-m mt-l mb-l fill nowrap`）与**模板专属**（`terminal bar dot kw fn str cmt num tag agenda-row speaker av t d`）。
...
适用 role：cover。
内容约束：主标题 ≤14 字（可一行或两行 `<br>` 分行）；lede ≤28 字；kicker 是主题标签（如 `tech-sharing / 2026-09`）。

```html
<section class="slide" data-layout="cover">
  <p class="kicker">{{主题标签}}</p>
  <h1 class="h1">{{主标题，两行用 <br> 分隔}}</h1>
  <p class="lede mt-m">{{一句话定位，≤28 字}}</p>
...
用途：目录/议程。编号行列表，每行标题 + 预计时长。
适用 role：toc。
内容约束：4-6 行；每行标题 ≤18 字；d 列写预计时长（如 `~5min`）。
...
<section class="slide" data-layout="agenda">
  ...
  <div class="stack mt-l">
    <div class="agenda-row"><span class="num">01</span><span class="t">{{条目标题}}</span><span class="d">~{{时长}}min</span></div>
    <div class="agenda-row"><span class="num">02</span><span class="t">{{条目标题}}</span><span class="d">~{{时长}}min</span></div>
  </div>
  <div class="notes">{{讲稿}}</div>
</section>
```
...
用途：章节过渡。一页只说一件事：进入第几章、这章讲什么。
适用 role：divider。
内容约束：kicker 写章节进度（如 `part · 1 / 3`）；标题 ≤16 字；可选一行 lede ≤24 字。

```html
<section class="slide" data-layout="section-divider">
  <p class="kicker">{{章节进度}}</p>
  <h2 class="h2">{{章节标题}}</h2>
  <p class="lede mt-m">{{本章要解决的一句问题}}</p>
  <div class="notes">{{讲稿}}</div>
</section>
```
...
适用 role：content。
内容约束：恰好 3 卡；卡标题 ≤12 字；说明 ≤40 字；每卡一个结论 tag（≤8 字）。
...
<section class="slide" data-layout="cards-3">
  ...
  <div class="grid g3 mt-l">
    <div class="card card-accent"><h4>{{卡标题}}</h4><p class="dim">{{说明}}</p><span class="tag mt-s">{{结论标签}}</span></div>
    <div class="card card-accent"><h4>{{卡标题}}</h4><p class="dim">{{说明}}</p><span class="tag mt-s">{{结论标签}}</span></div>
    <div class="card card-accent"><h4>{{卡标题}}</h4><p class="dim">{{说明}}</p><span class="tag mt-s">{{结论标签}}</span></div>
  </div>
  <div class="notes">{{讲稿}}</div>
...
用途：左边把概念讲透，右边放一小段代码/命令/输出佐证。
适用 role：content / code。
内容约束：左侧 lede ≤60 字 + ≤4 个 tag；terminal 内代码 ≤16 行、每行 ≤60 字符。
...
<section class="slide" data-layout="split-terminal">
  ...
  <div class="grid g2 mt-l" style="align-items:start">
    <div>
      <p class="lede">{{概念讲解，代码名词用 <span class="mono">}}</p>
      <div class="mt-l">
...
合法类名：slide, kicker, h2, grid, g2, mt-l, lede, tag, terminal, bar, dot, kw, fn, str, cmt, num, mono, notes
...
适用 role：code。
内容约束：代码 ≤22 行、每行 ≤70 字符；kicker 写文件名与行数（如 `runtime.rs · ~40 LOC`）；标题 ≤16 字。
...
<section class="slide" data-layout="code-terminal">
  ...
  <div class="terminal mt-m">
    <div class="bar"><span class="dot"></span><span class="dot"></span><span class="dot"></span><span>{{文件名}}</span></div>
<pre><span class="kw">{{代码，语法着色用 kw/fn/str/cmt/num}}</span></pre>
  </div>
  <div class="notes">{{讲稿}}</div>
</section>
```

合法类名：slide, kicker, h2, terminal, mt-m, bar, dot, kw, fn, str, cmt, num, notes
...
适用 role：content。
内容约束：恰好 2 列；每列卡内标题 ≤10 字、说明 ≤48 字；可用 tag 行给结论。
...
<section class="slide" data-layout="two-column">
  ...
  <div class="grid g2 mt-l">
    <div class="card"><h4>{{列标题A}}</h4><p class="dim">{{说明}}</p><span class="tag mt-s">{{结论}}</span></div>
    <div class="card card-accent"><h4>{{列标题B}}</h4><p class="dim">{{说明}}</p><span class="tag mt-s">{{结论}}</span></div>
  </div>
  <div class="notes">{{讲稿}}</div>
</section>
```
...
适用 role：data。
内容约束：主数字 ≤8 字符（用 `.mono`，可带单位）；指标名 ≤14 字；支撑要点 2-3 条、每条 ≤20 字。数字必须有出处，没有就用区间或量级。
...
<section class="slide" data-layout="stat-hero">
  ...
  <h2 class="h2 mt-m">{{指标名}}</h2>
  <div class="stack mt-m">
    <span class="tag">{{支撑要点}}</span>
    <span class="tag">{{支撑要点}}</span>
  </div>
  <div class="notes">{{讲稿}}</div>
</section>
...
内容约束：恰好 4 卡；卡标题（数字或特性名）≤10 字符；说明 ≤24 字。
...
<section class="slide" data-layout="kpi-grid">
  ...
  <div class="grid g4 mt-l">
    <div class="card card-accent"><h4 class="mono" style="color:var(--accent)">{{数字/特性}}</h4><p class="dim">{{说明}}</p></div>
    <div class="card card-accent"><h4 class="mono" style="color:var(--accent)">{{数字/特性}}</h4><p class="dim">{{说明}}</p></div>
    <div class="card card-accent"><h4 class="mono" style="color:var(--accent)">{{数字/特性}}</h4><p class="dim">{{说明}}</p></div>
    <div class="card card-accent"><h4 class="mono" style="color:var(--accent)">{{数字/特性}}</h4><p class="dim">{{说明}}</p></div>
  </div>
...
适用 role：quote。
内容约束：引文 ≤40 字；出处 ≤20 字；不用编造的引用，没有真实引文就换版式。
...
用途：版本演进、里程碑、历史脉络。
适用 role：content。
内容约束：4-6 个节点；每节点 = 时间点（mono）+ 事件 ≤16 字。
...
<section class="slide" data-layout="timeline">
  ...
  <h2 class="h2">{{标题}}</h2>
  <div class="stack mt-l">
    <div class="agenda-row"><span class="num">{{时间点}}</span><span class="t">{{事件}}</span></div>
    <div class="agenda-row"><span class="num">{{时间点}}</span><span class="t">{{事件}}</span></div>
  </div>
  <div class="notes">{{讲稿}}</div>
</section>
...
适用 role：content。
内容约束：3-5 步；每步 = 步骤号 + 动词短语 ≤14 字 + 说明 ≤20 字。
...
<section class="slide" data-layout="process-steps">
  ...
  <h2 class="h2">{{流程名}}</h2>
  <div class="stack mt-l">
    <div class="agenda-row"><span class="num">1.</span><span class="t">{{动词短语}}</span><span class="d">{{补充说明}}</span></div>
    <div class="agenda-row"><span class="num">2.</span><span class="t">{{动词短语}}</span><span class="d">{{补充说明}}</span></div>
  </div>
  <div class="notes">{{讲稿}}</div>
</section>
```
...
适用 role：content / cta。
内容约束：恰好 3 卡；卡标题 ≤12 字；说明 ≤32 字；延伸阅读行 ≤40 字（真实的链接，没有就省略整行）。
...
<section class="slide" data-layout="takeaways-3">
  ...
  <div class="grid g3 mt-l">
    <div class="card card-accent"><h4>{{记忆点1}}</h4><p class="dim">{{说明}}</p></div>
    <div class="card card-accent"><h4>{{记忆点2}}</h4><p class="dim">{{说明}}</p></div>
    <div class="card card-accent"><h4>{{记忆点3}}</h4><p class="dim">{{说明}}</p></div>
  </div>
...
适用 role：thanks / cta。
内容约束：主文案 ≤10 字；lede ≤30 字；标签 ≤2 个。
...
<section class="slide center tc" data-layout="qa">
  <div>
    ...
    <h2 class="h2">{{主文案}}</h2>
    <p class="lede" style="margin:14px auto">{{一句话收尾}}</p>
